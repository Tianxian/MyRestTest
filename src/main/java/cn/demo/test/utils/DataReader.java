...
public class DataReader {
	 protected static final Logger logger = LoggerFactory.getLogger(DataReader.class);
	  ...
	  public DataReader(XSSFSheet sheet, Boolean has_headers, Boolean has_key_column, Integer key_column) {
	    ...
	    this.byColumnName = has_headers;//true
	    this.byRowKey = has_key_column;//true
	    
	    try {
	    
	      if(byColumnName) {
	        myRow = sheet.getRow(0);//get first row
	        for(Cell cell: myRow) {
	          headers.add(cell.getStringCellValue());//get column names and add them to headers List
	        }
	        size = 1;
	      }
	  
	      for(; (myRow = sheet.getRow(size)) != null; size++ ) {//for each row in the sheet
	  
	        myList = new HashMap<String, String>();
	  
	        if(byColumnName) {//true
	          for(int col = 0; col < headers.size(); col++ ) {
	            if(col < myRow.getLastCellNum()) {
	              myList.put(headers.get(col), getSheetCellValue(myRow.getCell(col))); // put column name and value of current row to HashMap<String, String> myList 
	            } else {
